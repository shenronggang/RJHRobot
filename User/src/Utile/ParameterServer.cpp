...
void ParameterServer::getRobotInfo(RobotData::RobotState &robot_state)
{
    ...
    robot_state = robot_info_->robot_state_; // 复制当前状态
}
...
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    robot_cmd = robot_info_->robot_cmd_; // 复制当前关节指令
}
// 获取关节指令
void ParameterServer::getRobotInfo(RobotData::JointCmd &joint_cmd)
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    joint_cmd = robot_info_->joint_cmd_; // 复制当前关节指令
}
...
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    *robot_info_ = newInfo; // 直接修改指针指向的对象
}
...
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    robot_info_->robot_state_ = newState; // 修改机器人状态
}
...
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    robot_info_->robot_send_info_ = newSendInfo; // 修改发送信息
}
...
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    robot_info_->robot_cmd_ = newCmd; // 修改指令
}
...
{
    std::lock_guard<std::mutex> lock(dataMutex_);
    robot_info_->joint_cmd_ = newJointCmd; // 修改关节指令
}
